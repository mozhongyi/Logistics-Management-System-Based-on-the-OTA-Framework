...
public:	//定义接口
	...
	// 定义添加场地接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_POST, "/SpaceMgr/addspace", addSpace, BODY_DTO(SpaceAddDTO::Wrapper, dto), execAddSpace(dto, authObject->getPayload()));

	// 定义修改场地接口描述
	API_DEF_ENDPOINT_INFO_AUTH(ZH_WORDS_GETTER("space.modify-space.summary"), modifySpace, StringJsonVO::Wrapper);
	// 定义修改场地接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_PUT, "/spacemgr/modifyspace", modifySpace, BODY_DTO(SpaceUpdateDTO::Wrapper, dto), execModifySpace(dto, authObject->getPayload()));
	
	// 删除场地接口描述
	API_DEF_ENDPOINT_INFO_AUTH(ZH_WORDS_GETTER("space.delete-space.summary"), removeSpace, ListJsonVO<String>::Wrapper);
	// 定义删除接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_DEL, "/spacemgr/removespace", removeSpace, BODY_DTO(List<String>, ids), execRemoveSpace(ids));
